...
import { SCHEMA_V1, SCHEMA_V2, SCHEMA_V3, SCHEMA_V4, SCHEMA_V5, SCHEMA_V6, SCHEMA_V7, SCHEMA_V8, SCHEMA_V9, SCHEMA_V10, SCHEMA_V11, SCHEMA_V12, SCHEMA_V13, SCHEMA_V14, SCHEMA_V15, SCHEMA_V16, SCHEMA_V17, SCHEMA_V18, SCHEMA_V19, SCHEMA_V20, SCHEMA_V21, SCHEMA_V22, SCHEMA_V23, SCHEMA_V24, SCHEMA_V25, SCHEMA_V26, SCHEMA_V27, SCHEMA_V28, SCHEMA_V29, SCHEMA_V30, SCHEMA_V31, SCHEMA_V32, SCHEMA_V33, SCHEMA_V34, SCHEMA_V35, SCHEMA_V36, SCHEMA_V37, SCHEMA_V38, SCHEMA_V39, SCHEMA_V40, SCHEMA_V41, SCHEMA_V42, SCHEMA_V43, SCHEMA_V44, SCHEMA_V45, SCHEMA_V46, SCHEMA_V47, SCHEMA_V48, SCHEMA_V49, SCHEMA_V50, SCHEMA_V51, SCHEMA_V52, SCHEMA_V53, SCHEMA_V54, SCHEMA_V55, SCHEMA_V56, SCHEMA_V57, SCHEMA_V58 } from './schema'
...
export function runMigrations(db: Database.Database) {
  ...
  if (currentVersion < 23) {
    db.transaction(() => {
      const stmts = SCHEMA_V23.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('already exists')) throw err
      ...
      db.prepare('INSERT INTO _migrations (version) VALUES (?)').run(23)
    })()
  }

  if (currentVersion < 24) {
    db.transaction(() => {
      const stmts = SCHEMA_V24.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('duplicate column') && !msg.includes('already exists')) throw err
  ...
  if (currentVersion < 47) {
    db.transaction(() => {
      const stmts = SCHEMA_V47.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('already exists')) throw err
      ...
      db.prepare('INSERT INTO _migrations (version) VALUES (?)').run(47)
    })()
  }

  if (currentVersion < 48) {
    db.transaction(() => {
      const stmts = SCHEMA_V48.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('already exists')) throw err
      ...
      db.prepare('INSERT INTO _migrations (version) VALUES (?)').run(48)
    })()
  }

  if (currentVersion < 49) {
    db.transaction(() => {
      const stmts = SCHEMA_V49.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('duplicate column') && !msg.includes('already exists')) throw err
      ...
      db.prepare('INSERT INTO _migrations (version) VALUES (?)').run(49)
    })()
  }

  if (currentVersion < 50) {
    db.transaction(() => {
      const stmts = SCHEMA_V50.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('duplicate column') && !msg.includes('already exists')) throw err
  ...
  if (currentVersion < 52) {
    db.transaction(() => {
      const stmts = SCHEMA_V52.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('duplicate column') && !msg.includes('already exists')) throw err
      ...
      db.prepare('INSERT INTO _migrations (version) VALUES (?)').run(52)
    })()
  }

  if (currentVersion < 53) {
    db.transaction(() => {
      const stmts = SCHEMA_V53.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('duplicate column') && !msg.includes('already exists')) throw err
      ...
      db.prepare('INSERT INTO _migrations (version) VALUES (?)').run(53)
    })()
  }

  if (currentVersion < 54) {
    db.transaction(() => {
      const stmts = SCHEMA_V54.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('duplicate column') && !msg.includes('already exists')) throw err
      ...
      db.prepare('INSERT INTO _migrations (version) VALUES (?)').run(54)
    })()
  }
  ...
  if (currentVersion < 61) {
    db.transaction(() => {
      const stmts = SCHEMA_V58.split(';').map((s) => s.trim()).filter(Boolean)
      for (const stmt of stmts) {
        try { db.exec(stmt) } catch (err) {
          const msg = (err instanceof Error ? err.message : String(err)).toLowerCase()
          if (!msg.includes('already exists')) throw err
