...
export function registerProcessHandlers() {
  ...
  ipcMain.handle('process:list', ipcHandler(async () => {
    const db = getDb()
  ...
  ipcMain.handle('process:orphans', ipcHandler(async () => {
    ...
    if (process.platform === 'win32') {
      const output = await new Promise<string>((resolve) => {
        exec('tasklist /FO CSV /NH', { encoding: 'utf8', timeout: TIMEOUTS.TASKLIST }, (err, stdout) => {
          resolve(err ? '' : stdout)
        })
      })
  ...
  // Kill a process
  ipcMain.handle('process:kill', ipcHandler(async (_event, pid: number) => {
    ...
    if (!isManagedSession && !isKnownOrphan) {
      throw new Error('PID not recognized as a managed or orphaned process')
    }

    if (process.platform === 'win32') {
      // Graceful first — use execFile (no shell) to prevent injection
      await new Promise<void>((resolve) => {
        execFile('taskkill', ['/PID', String(pid)], { timeout: TIMEOUTS.TASKKILL }, () => resolve())
      })

      // Check if still alive after 3s
      await new Promise((r) => setTimeout(r, TIMEOUTS.KILL_GRACE_PERIOD))
      try {
        await pidusage(pid)
        // Still alive — force kill
        execFile('taskkill', ['/PID', String(pid), '/F'], { timeout: TIMEOUTS.TASKKILL })
      } catch {
        // Already dead — good
      }
    } else {
      ...
      try { process.kill(pid, 'SIGKILL') } catch (err) {
        console.warn('[Processes] SIGKILL failed (process likely already dead):', (err as Error).message)
      }
    }
  }))
}
