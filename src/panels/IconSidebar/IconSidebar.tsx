...
export function IconSidebar({ showExplorer, onToggleExplorer, onOpenAgentLauncher }: IconSidebarProps) {
  ...
  useEffect(() => {
    const handlePointerDown = (event: MouseEvent) => {
      if (!showPluginsMenu) return
      if (pluginsMenuRef.current?.contains(event.target as Node)) return
  ...
  // Panels that replace center content (editor disappears)
  ...
  const handlePluginsClick = () => {
    ...
    setShowPluginsMenu((value) => !value)
  }
  ...
  return (
    <aside className="icon-sidebar">
      ...
      <div className="sidebar-submenu-wrap" ref={pluginsMenuRef}>
        <button
        ...
        {showPluginsMenu && (
          <div className="sidebar-submenu" role="menu" aria-label="Plugins submenu">
            <div className="sidebar-submenu-group">Web3 Tools</div>
            <button className="sidebar-submenu-item" onClick={handleRecoveryClick}>
              Wallet Recovery
            </button>
          </div>
        )}
